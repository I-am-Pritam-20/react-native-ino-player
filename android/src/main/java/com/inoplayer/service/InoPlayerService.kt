...
import com.inoplayer.player.CarBrowseRegistry
...
class InoPlayerService : MediaLibraryService() {
    inner class LocalBinder : Binder() { internal fun controller(): PlayerController = playerController }
    ...
    companion object {
        ...
        fun getOrCreateCache(context: android.content.Context, dir: File, maxBytes: Long): SimpleCache = _cache ?: synchronized(this) {
            _cache ?: SimpleCache(File(dir, "ino_media_cache"), LeastRecentlyUsedCacheEvictor(maxBytes), StandaloneDatabaseProvider(context)).also { _cache = it }
        }
    }

    override fun onCreate() {
        super.onCreate()
        val cache = getOrCreateCache(this, cacheDir, 1_073_741_824L)
