...
export class WebPlayer {
  ...
  async updateOptions(options: UpdateOptions): Promise<void> {
    ...
    if (options.jumpForwardInterval !== undefined)
      this._jumpForward = options.jumpForwardInterval;
    if (options.jumpBackwardInterval !== undefined)
      this._jumpBackward = options.jumpBackwardInterval;
  ...
  async seekBy(offset: number): Promise<void> {
    ...
    const newPos = Math.max(
      0,
      Math.min(this._el.currentTime + offset, this._el.duration || Infinity)
    );
  ...
  showAirPlayPicker(): void {
    ...
    console.warn(
      '[InoPlayer Web] AirPlay picker must be opened via browser native controls on web.'
    );
  }
  ...
  showCastDialog(): void {
    ...
    console.warn(
      '[InoPlayer Web] Chromecast on web requires the Google Cast SDK. See docs/ARCHITECTURE.md.'
    );
  }
  ...
  private _loadCurrentTrack(initialPosition = 0): void {
    ...
    if (track.headers && Object.keys(track.headers).length > 0) {
      ...
      console.warn(
        '[InoPlayer Web] HTTP headers on media elements are not directly supported by the browser. Use signed URLs or a Service Worker proxy instead.'
      );
    }
      ...
      case RepeatMode.Off:
        if (this._currentIndex === this._queue.size - 1) {
          ...
          emit(Event.PlaybackQueueEnded, {
            index: this._currentIndex,
            position: this._el?.currentTime ?? 0,
          });
        } else {
          this._currentIndex++;
          this._loadCurrentTrack();
  ...
  private _playOrder(): number[] {
    if (this._shuffleEnabled) {
      if (this._shuffledOrder.length !== this._queue.size)
  ...
  private _previousIndex(): number | null {
    ...
    return this._repeatMode === RepeatMode.Queue
